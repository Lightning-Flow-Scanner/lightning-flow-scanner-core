...
describe("When there is a a rule override",async function () {
  ...
  before("arrange",  async function () {
    ...
    ruleOptions = new ScannerOptions(undefined, [
      new FlowScanOverrides('Create Property', [
        new Override('DuplicateDMLOperationsByNavigation', 'error_creating_records'),
        new Override('DuplicateDMLOperationsByNavigation', 'upload_picture'),
        new Override('DuplicateDMLOperationsByNavigation', 'navigate_to_record_detail'),
      ])]);
  ...
  it("there is no result for that override", async function () {
    ...
    expect(scanResults[0].ruleResults.find(result => result.ruleName === "DuplicateDMLOperationsByNavigation").occurs).to.equal(false);
